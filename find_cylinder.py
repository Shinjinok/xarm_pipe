...
class FindCylinder():
    def __init__(self):
        try:
            ...
            self.MaxX = parser.get('Find Cylinder Param', 'MaxX')
            self.MinX = parser.get('Find Cylinder Param', 'MinX')
            self.MaxZ = parser.get('Find Cylinder Param', 'MaxZ')
            self.MinZ = parser.get('Find Cylinder Param', 'MinZ')
            self.MaxR = parser.get('Find Cylinder Param', 'MaxR')

        except:
            
            print('Find Cylinder Param error, check robot.conf. Program exit')
            sys.exit(1)



    def get_flattened_pcds2(self, source,A,B,C,D,x0,y0,z0):
    ...
    def get_cylinder(self, pcd,thresh=0.1, maxIteration=1000):
    ...
    #    o3d.visualization.draw_geometries([outlier_cloud])
        ...
        #o3d.visualization.draw_geometries([outlier_cloud])
            ...
            print("vecC: " + str(nor))
            if (rad < self.MaxR and cen[0] > self.MinX and cen[0] < self.MaxX and cen[2] > self.MinZ and cen[2] < self.MaxZ):
                center.append(cen)
                normal.append(nor)
                radius.append(rad)
    ...
    def get_clylinder_mesh(self, center, normal, radius):
    ...
    def get_clylinder_pos(self, center, normal):
        r = pyrsc.get_rotationMatrix_from_vectors([0, 0, 1], normal)
        R = np.identity(4)
        R[0:3,0:3] = r
        print("R :",np.array(R))
    ...
    def get_plane(self, pcd2,thresh=0.1,maxIteration=1000):
        cil = pyrsc.Plane()
